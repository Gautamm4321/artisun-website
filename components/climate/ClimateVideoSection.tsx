...
export default function ClimateVideoSection() {
  ...
  const titleWordsRefs = useRef<(HTMLSpanElement | null)[][]>(Array.from({ length: 5 }, () => []));
  const subtitleWordsRefs = useRef<(HTMLSpanElement | null)[][]>(Array.from({ length: 5 }, () => []));
  ...
  useEffect(() => {
    ...
    const locate = (rawUnits: number) => {
      ...
      if (seg.type === 'hold') {
        return { fromIdx: seg.index, toIdx: seg.index, e: 0, isHold: true, segProgress: local };
      }
      return { fromIdx: seg.index, toIdx: seg.index + 1, e: cinematicEase(local), isHold: false, segProgress: local };
    };
    ...
    const setRest = (i: number, active: boolean, segProgress: number = 0) => {
      ...
      gsap.set(titleRefs.current[i], { autoAlpha: active ? 1 : 0 });
      gsap.set(subtitleRefs.current[i], { autoAlpha: active ? 1 : 0 });
      
      const words = [...titleWordsRefs.current[i], ...subtitleWordsRefs.current[i]];
      const total = words.length;
      words.forEach((w, idx) => { 
        if (!w) return;
        if (active) {
          // Sequentially light up words over the first 80% of the hold phase
          const staggerStart = (idx / total) * 0.8;
          const localE = clamp01((segProgress - staggerStart) / 0.2);
          w.style.opacity = (0.15 + 0.85 * localE).toString();
        } else {
    ...
    const applyOut = (i: number, e: number) => {
      ...
      const words = [...titleWordsRefs.current[i], ...subtitleWordsRefs.current[i]];
      words.forEach(w => { if (w) w.style.opacity = '1'; }); // Fully lit as it fades away
    ...
    const applyIn = (i: number, e: number) => {
      ...
      // Keep containers visible so the 15% opacity words can be seen sliding in
      gsap.set(titleRefs.current[i], { autoAlpha: 1 });
      gsap.set(subtitleRefs.current[i], { autoAlpha: 1 });

      const words = [...titleWordsRefs.current[i], ...subtitleWordsRefs.current[i]];
      words.forEach((w) => {
        if (w) w.style.opacity = '0.15'; // Keep dim while dollying in
      });
    ...
    const update = (progress: number) => {
      const { fromIdx, toIdx, e, isHold, segProgress } = locate(progress * totalLen);
      ...
      for (let i = 0; i < N; i++) {
        if (isHold) {
          setRest(i, i === fromIdx, segProgress);
        } else if (i === fromIdx) {
          applyOut(i, e);
        } else if (i === toIdx) {
          applyIn(i, e);
  ...
  return (
    <section ref={containerRef} className="relative w-full h-[100svh] bg-transparent z-20">
      <div
        ref={wrapperRef}
        className="relative w-full h-[100svh] overflow-hidden z-0 bg-black origin-center"
                ...
                >
                  ...
                  >
                    <h2
                      ...
                      className="flex flex-wrap font-editorial font-normal text-[26px] md:text-[38px] lg:text-[50px] leading-[1.15] tracking-[-0.01em] text-[#a62a2c] drop-shadow-[0_4px_16px_rgba(255,255,255,0.6)] gap-x-[0.25em] gap-y-[0.15em]"
                      style={{ opacity: 0, willChange: 'transform, opacity' }}
                    >
                      {title.split(" ").map((word, wordIndex) => (
                        <span
                          key={`tw-${i}-${wordIndex}`}
                          ref={(el) => { titleWordsRefs.current[i][wordIndex] = el; }}
                    ...
                    <p
                      ...
                      className="flex flex-wrap font-editorial font-normal text-[26px] md:text-[38px] lg:text-[50px] leading-[1.15] tracking-[-0.01em] text-[#a62a2c]/85 drop-shadow-[0_4px_16px_rgba(255,255,255,0.5)] gap-x-[0.25em] gap-y-[0.15em] mt-1 md:mt-2"
                      style={{ opacity: 0, willChange: 'transform, opacity' }}
                    >
                      {subtitle.split(" ").map((word, wordIndex) => (
                        <span
                          key={`sw-${i}-${wordIndex}`}
                          ref={(el) => { subtitleWordsRefs.current[i][wordIndex] = el; }}
                          className="opacity-[0.15]"
                        >
                          {word}
                        </span>
