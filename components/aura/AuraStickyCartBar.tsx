...
export default function AuraStickyCartBar() {
  ...
  useEffect(() => {
    ...
    const hero = document.querySelector('.aura-panel');
    if (!hero) return;
    ...
    >
      <div className="h-full max-w-[1500px] mx-auto px-4 sm:px-8 lg:px-14 flex items-center gap-3 sm:gap-4">
        ...
        <div className="relative h-10 w-10 rounded-lg overflow-hidden shrink-0 border border-[var(--brand-cream)]/15">
          <Image src={asset('/about-media/aura-1.jpg')} alt="Aura" fill sizes="40px" className="object-cover" />
        </div>
        <div className="min-w-0">
          <div className="font-editorial text-[var(--brand-cream)] text-sm sm:text-base leading-none">AURA</div>
          <div className="hidden sm:block font-suisse text-[10px] uppercase tracking-[0.14em] text-[var(--brand-cream)]/50 mt-1.5">
            Pearl Skinwear SPF 40
          </div>
        </div>

        {/* Right cluster */}
        <div className="ml-auto flex items-center gap-2.5 sm:gap-4">
          <span className="font-editorial text-[var(--brand-cream)] text-base sm:text-lg">₹1,799</span>
